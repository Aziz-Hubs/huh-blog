  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-12 sm:px-6 sm:py-16">
      ...
      <div className="mt-8 flex flex-wrap gap-2 text-sm">
        <Link href="/blog" className={!tag ? "rounded-full bg-primary px-3 py-1 text-primary-foreground" : "warm-chip rounded-full px-3 py-1 text-muted-foreground hover:text-foreground"}>All</Link>
        {tags.map((item) => (
          <Link key={item.slug} href={`/blog?tag=${item.slug}`} className={tag === item.slug ? "rounded-full bg-primary px-3 py-1 text-primary-foreground" : "warm-chip rounded-full px-3 py-1 text-muted-foreground hover:text-foreground"}>
            {item.name}
          </Link>
        ))}
      </div>
